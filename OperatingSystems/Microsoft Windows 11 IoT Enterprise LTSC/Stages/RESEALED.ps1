# RESEALED.ps1 - Subscript for YUNA (Yielding Universal Node Automation)

# This script is responsible for:
# - Ensuring the current OS is set before execution
# - Running RESEALED-specific PowerShell scripts
# - Importing RESEALED-specific registry modifications
...
# Define the path to the RESEALED scripts folder
$scriptsFolder = Join-Path -Path "$PSScriptRoot/RESEALED" -ChildPath "scripts"

# Check if the scripts folder exists before executing scripts
if (Test-Path $scriptsFolder) {
    Write-Log "Executing RESEALED-specific scripts from: $scriptsFolder" "INFO"
...
# Define the path to the RESEALED registry folder
$registryFolder = Join-Path -Path "$PSScriptRoot/RESEALED" -ChildPath "registry"

if (Test-Path $registryFolder) {
    Write-Log "Importing RESEALED-specific registry files from: $registryFolder" "INFO"
