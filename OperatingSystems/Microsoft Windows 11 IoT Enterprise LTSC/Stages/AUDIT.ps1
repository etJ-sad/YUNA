# AUDIT.ps1 - Subscript for YUNA (Yielding Universal Node Automation)

# This script is responsible for:
# - Ensuring the current OS is set before execution
# - Running AUDIT-specific PowerShell scripts
# - Importing AUDIT-specific registry modifications
...
# Define the path to the AUDIT scripts folder
$scriptsFolder = Join-Path -Path "$PSScriptRoot/AUDIT" -ChildPath "scripts"

# Check if the scripts folder exists before executing scripts
if (Test-Path $scriptsFolder) {
    Write-Log "Executing AUDIT-specific scripts from: $scriptsFolder" "INFO"
...
# Define the path to the AUDIT registry folder
$registryFolder = Join-Path -Path "$PSScriptRoot/AUDIT" -ChildPath "registry"

if (Test-Path $registryFolder) {
    Write-Log "Importing AUDIT-specific registry files from: $registryFolder" "INFO"
