# REMOVE_ALL_HIDDEN_DEVICES.ps1 - Subscript for YUNA (Yielding Universal Node Automation)
#
# This script is responsible for:
# - Accepting parameters to filter devices based on class and friendly name.
# - Enumerating all system devices using the Windows Setup API.
# - Filtering devices using criteria provided via parameters:
#     • FilterByClass and NarrowByClass for device classes.
#     • FilterByFriendlyName and NarrowByFriendlyName for device names.
# - Offering options to either list devices (or ghost devices) only,
#   or to proceed with device removal.
# - Detecting ghost devices (devices with an InstallState of false) and, if permitted,
#   prompting the user for confirmation (unless forced) to remove these devices.
# - Logging and reporting the outcome by displaying filtered devices and a summary
#   of any devices that have been removed.
# - Leveraging embedded C# code (via Add-Type) to interface with native Setup API functions,
#   facilitating device enumeration, property retrieval, and removal.
